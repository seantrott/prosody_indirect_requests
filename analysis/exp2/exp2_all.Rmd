...
```{r}
coded %>%
  ggplot(aes(x = condition, y = numeric_response)) +
...
### Does form also predict response?

```{r}

m1_with_form = glmer(data=coded, response ~ condition + form +
                       (1 + condition | stimNum) +
             (1 + condition + form | subject),
           control=glmerControl(optimizer="bobyqa"),
         family=binomial())

m1_base = glmer(data=coded, response ~ condition + (1 + condition | stimNum) +
             (1 + condition + form | subject),
           control=glmerControl(optimizer="bobyqa"),
         family=binomial())
...
anova(m1_with_form, m1_base)

summary(m1_with_form)
```
...
model_response_reduced = glmer(data = merged_with_means,
                      response ~ form + condition +
                        (1 + condition + form + mean_request | subject),
                       control=glmerControl(optimizer="bobyqa"),
                       family = binomial())

anova(model_response_full, model_response_reduced)
